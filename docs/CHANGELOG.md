...
### Přidáno
- **Katalog zdrojů** ([katalog.zpravobot.news](https://katalog.zpravobot.news)) — statický web se všemi zdroji instance, generovaný dávkově a nahrávaný na Surfer. `Catalog::DataAggregator` (spojení 3 zdrojů: `mastodon_accounts.yml` + `sources/*.yml` + Mastodon API/DB snapshot) → `Catalog::Renderer` → statické soubory. Frontend vanilla JS: filtry (oblast/typ/jazyk/tag + full-text vč. tagů), řezy (platforma, Top N sledovaných/aktivních, skokani týdne, nedávno přidané), detail modal s hover preview, kontextové štítky typu (`team`/Instituce/Organizace), řazení, stav v URL hash, responsivní accordion. **SEO**: OG/Twitter card, canonical, `sitemap.xml`, `robots.txt`, per-účet sdílecí stuby `zdroj/<id>.html`. **I18n**: přepínač CZ\|EN (default CZ, EN = UI shell). `bin/build_catalog.rb` (+ `--upload-test`/`--no-upload`/`--no-stubs`), `bin/catalog_dump.rb`; URL katalogu v `global.yml` `infrastructure.catalog_*_url`. Cron neděle 20:30. Viz [`40-tools/catalog.md`](40-tools/catalog.md)
...
#### Zabezpečení
- **B1** — webhook auth fail-closed: `bin/ifttt_webhook.rb` při chybějícím `IFTTT_AUTH_TOKEN` nebo `TLAMBOT_WEBHOOK_SECRET` skončí `exit 1` místo dřívějšího warn + start. `SignatureVerifier.verify_broadcast_signature` a `IftttRoute#call` doplněny o defense-in-depth: bez secretu / tokenu vrací `false` / `401` (předtím fail-open: `return true unless secret`). Flip proveden po týdnu monitoringu WARN-only módu (nasazeno během B2 a paralelně se zaváděním auth tokenů do IFTTT appletů) — žádné stížnosti na 401 nepřišly. ADR-055 aktualizován (smazán odstavec o přechodném stavu).
- **B2** — ADR-055 přepsán: `IFTTT_BIND="0.0.0.0"` je vědomé rozhodnutí (Cloudron topologie — nginx běží na hostu mimo kontejner, loopback bind by ho odřízl; ověřeno 13h výpadkem 2026-05-23 18:44 → 2026-05-24 07:46). HMAC povýšen z „defense-in-depth" na **primární autentizaci** broadcast endpointu.
